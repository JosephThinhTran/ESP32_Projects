...
Software timer is used in this implementation.
...
// Global vars
TimerHandle_t swTimer;  // software timer
...
void vTimerCb(TimerHandle_t xTimer) {
  ...
  if (gDBuf.wIdx >= BUF_LEN) {
    ...
    if (xSemaphoreTake(semProcDone, portMAX_DELAY) == pdFALSE) {
      gProcDone = false;
    }
...
void procAdc(void*) {
  ...
  // Timer creation
  swTimer = xTimerCreate("Software timer", 100 / portTICK_PERIOD_MS, true, (void*)0, vTimerCb);
